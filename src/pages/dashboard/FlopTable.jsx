...
export default function FlopTable({flop, tcard1, tcard2, tcard3, tcard_type1, tcard_type2, tcard_type3}) {
  ...
  if(flop.actions){    
    ...
    let actions = flop.actions.replace(/'/g, '"');
    actions = JSON.parse(actions);  
    let pos = flop.pos.replace(/'/g, '"');
    pos = JSON.parse(pos); 
    
    let stacks = flop.stacks.replace(/'/g, '"');
    stacks = JSON.parse(stacks);     
    let bets = flop.bets.replace(/'/g, '"');
    ...
    for(let i = 0; i < actions.length; i++){
      let b = '';
      if(bets[i])b = bets[i];
      rows.push(createData(pos[i], actions[i]+ " " +  b , stacks[i], flop.id));
    }
    ...
    <Box>
      ...
      >
        <Table aria-labelledby="tableTitle">
          <TableHead>
            <TableRow>
              <TableCell sx={{backgroundColor:'#EEE'}}>Flop</TableCell>
