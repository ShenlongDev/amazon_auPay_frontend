...
import { CrownOutlined, DeleteColumnOutlined } from '@ant-design/icons';
...
export default function RiverTable({ river, poker_info, tcard1, tcard2, tcard3, tcard4, tcard5, tcard_type1, tcard_type2, tcard_type3, tcard_type4, tcard_type5, round_info }) {
  console.log(river);
  if (river.actions) {

    let actions = river.actions.replace(/'/g, '"');
    actions = JSON.parse(actions);
    let pos = river.pos.replace(/'/g, '"');
    pos = JSON.parse(pos);

    let stacks = river.stacks.replace(/'/g, '"');
    stacks = JSON.parse(stacks);
    let bets = river.bets.replace(/'/g, '"');
    ...
    for (let i = 0; i < actions.length; i++) {
      let b = '';
      if (bets[i]) b = bets[i];
      rows.push(createData(pos[i], actions[i] + " " + b, stacks[i], river.id));
    }
      ...
      <Box>
        ...
        >
          <Table aria-labelledby="tableTitle">
            <TableHead>
              <TableRow>
                <TableCell sx={{ backgroundColor: '#EEE' }}>River</TableCell>
              ...
              <TableRow>
                <TableCell sx={{ backgroundColor: '#EEE' }} colSpan={3}>
                  <div style={{ display: 'flex', gap: '2px' }}>
                    ...
                    {tcard4}
                    <img src={`/assets/images/card/${tcard_type5}.png`} height={20} />
                    {tcard5}
                  </div>
                </TableCell>
              </TableRow>
              <TableRow>
                ...
                <TableCell sx={{ backgroundColor: '#EEE' }}>{river.ports}</TableCell>
                <TableCell sx={{ backgroundColor: '#EEE' }}></TableCell>
              </TableRow>
              <TableRow>
                ...
                rows.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>{row.bet != undefined ? row.bet : ''}</TableCell>
